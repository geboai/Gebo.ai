...
/**
 * AI generated comments This class provides methods to interact with Confluence
 * attachments on an on-premise server. It allows retrieving attachment content
...
public class OnPremiseConfluenceAttachmentApi {
	...
	/**
	 * Retrieves the content of a Confluence attachment as an InputStream, using a
	 * page ID and attachment item.
	 *
	 * @param pageId          the ID of the Confluence page containing the
	 *                        attachment
	 * @param attachementItem the attachment item metadata
	 * @return InputStream representing the attachment's content
	 * @throws GeboRestIntegrationException if there is an error accessing the
	 *                                      attachment
	 */
	...
	/**
	 * Retrieves the content of a Confluence attachment as an InputStream, using a
	 * content ID and attachment title.
	 *
	 * @param confluenceContentId the ID of the Confluence content containing the
	 *                            attachment
	...
	public OnPremiseConfluenceAttachmentItem getAttachment(String attachmentId) throws GeboRestIntegrationException {
		// Good default expansions for "metadata"
		return getAttachment(attachmentId, "version,container,space,history,metadata");
	}

	public OnPremiseConfluenceAttachmentItem getAttachment(String attachmentId, String expand)
			throws GeboRestIntegrationException {
		...
		// Optional: if you want to allow trashed/archived visibility depending on
		// permissions:
		// b.queryParam("status", "any");

		HttpEntity<Void> requestEntity = new HttpEntity<>(api.createHeaders());

		ResponseEntity<OnPremiseConfluenceAttachmentItem> response = api.restTemplate.exchange(b.toUriString(),
				HttpMethod.GET, requestEntity, OnPremiseConfluenceAttachmentItem.class);
		return response.getBody();
	}

	public InputStream getAttachmentContentByDownloadUrl(String downloadUrl) throws GeboRestIntegrationException {

		String url = RestTemplateWrapperService.urlConcat(api.getBaseUrl(), downloadUrl);
		HttpEntity requestEntity = new HttpEntity<OnPremiseConfluenceContent>(api.createHeaders());
		Class responseType = byte[].class;
		ResponseEntity<byte[]> response = api.restTemplate.exchange(url, HttpMethod.GET, requestEntity, responseType);
