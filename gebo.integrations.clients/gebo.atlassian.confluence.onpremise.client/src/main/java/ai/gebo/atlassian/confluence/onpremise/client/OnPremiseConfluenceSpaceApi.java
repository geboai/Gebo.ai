...
/**
 * AI generated comments Provides API methods to interact with Confluence Spaces
 * on-premise.
 */
@AllArgsConstructor
public class OnPremiseConfluenceSpaceApi {
	private static final String REST_API_RELATIVE_URL = "/rest/api/space";
	// Connection instance for interacting with Confluence's API
	protected final OnPremiseConfluenceConnection api;

	/**
	 * Retrieves a list of all Confluence spaces starting at a specific index and
	 * with a set limit.
	 *
	 ...
	 * @throws GeboRestIntegrationException if there is an issue with the API
	 *                                      request
	 */
	public OnPremiseConfluenceSpacesList getAllSpaces(Integer start, int limit, String type)
			throws GeboRestIntegrationException {
		String relative = REST_API_RELATIVE_URL;
		String url = api.getBaseUrl() + relative;
		if (type != null) {
			url += (urlParameterSeparator(url)) + "type=" + URLEncoder.encode(type);
		}
		if (start != null) {
			url += (urlParameterSeparator(url)) + "start=" + start;
		}
		if (limit > 0) {
			url += (urlParameterSeparator(url)) + "limit=" + limit;
		}
	...
	private String urlParameterSeparator(String url) {
	...
	public OnPremiseConfluenceSpacesList getSpace(String key) throws GeboRestIntegrationException {
		String relative = REST_API_RELATIVE_URL + "?spaceKey=" + URLEncoder.encode(key) + "&expand=homepage";
		String url = api.getBaseUrl() + relative;
		HttpEntity requestEntity = new HttpEntity<OnPremiseConfluenceSpacesList>(api.createHeaders());
		Class responseType = OnPremiseConfluenceSpacesList.class;
		ResponseEntity<OnPremiseConfluenceSpacesList> response = api.restTemplate.exchange(url, HttpMethod.GET,
