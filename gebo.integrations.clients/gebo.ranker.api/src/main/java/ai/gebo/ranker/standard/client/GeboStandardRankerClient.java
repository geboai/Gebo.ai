...
public class GeboStandardRankerClient implements RankerModel {
	...
	@Builder
	public GeboStandardRankerClient(String apiKey, String serviceUrl, String model,

			RestClient.Builder restClientBuilder, RetryTemplate retryTemplate,

			Integer modelContextWindowTokens, Integer maxDocumentsPerRequest, Integer maxDocumentTokens,
		...
		this.restClient = builder.baseUrl(this.serviceUrl).defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer " + apiKey)
	...
	public RankingOutput call(RankingInput input) {
		...
		if (documents.isEmpty()) {
			return RankingOutput.builder().ranked(List.of()).build();
		}

		int requestedTopK = input.getTopK() != null && input.getTopK() > 0 ? input.getTopK() : documents.size();
		...
		for (List<DocumentBatchItem> batch : batches) {
			...
			for (StandardRerankResponse.Result result : response.getResults()) {
				...
				allRanked.add(new RankingOutput.RankingItem(originalDocument, result.getRelevanceScore()));
			}
		}
	...
	private StandardRerankResponse execute(StandardRerankRequest request) {
		return restClient.post().body(request).retrieve().body(StandardRerankResponse.class);
	}
	...
	private List<DocumentBatchItem> prepareDocuments(List<Document> documents) {
		...
		for (int i = 0; i < documents.size(); i++) {
			...
			prepared.add(new DocumentBatchItem(i, document, truncated, estimateTokens(truncated)));
		}

		return prepared;
	}

	private List<List<DocumentBatchItem>> createBatches(String query, List<DocumentBatchItem> documents) {
		...
		for (DocumentBatchItem item : documents) {
			...
			if (item.estimatedTokens() > availableTokens) {
				...
				DocumentBatchItem reduced = new DocumentBatchItem(item.originalIndex(), item.originalDocument(),
						truncated, estimateTokens(truncated));
	...
	private StandardRerankRequest toRequest(String query, List<DocumentBatchItem> batch, int topK) {
		List<String> documents = batch.stream().map(DocumentBatchItem::text).toList();

		return StandardRerankRequest.builder().model(model).query(query).documents(documents)
				.topN(Math.min(topK, documents.size())).returnDocuments(false).build();
	}
	...
	private record DocumentBatchItem(int originalIndex, Document originalDocument, String text, int estimatedTokens) {
	}
