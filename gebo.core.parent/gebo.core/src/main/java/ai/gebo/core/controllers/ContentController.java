...
public class ContentController {
	...
	public void get(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			boolean asDownload = request.getParameter("asDownload") != null
					&& request.getParameter("asDownload").equalsIgnoreCase("true"); // Check if content should be
																					// downloaded
			String knowledgeBasesIds[] = request.getParameterValues("knowledgeBasesIds"); // Retrieve the knowledge base
																							// IDs

			Optional<GDocumentReference> documentR = documentRepository.findById(contentCode); // Find the document
			...
			if (documentR.isEmpty()) {
				...
				GProjectEndpoint endpoint = persistentObjectManager
						.findByReference(document.getProjectEndpointReference(), GProjectEndpoint.class); // Find
																											// project
																											// endpoint
				...
				if (handler == null) {
					throw new RuntimeException("Cannot find the content handler for the project endpoint => "
							+ document.getProjectEndpointReference().toString()); // Throw exception if handler not
																					// found
				}
				TypedInputStream stream = handler.streamContent(StreamingPurpose.SERVING, document, new HashMap<>());
				if (stream == null || stream.getInputStream() == null) {
					throw new RuntimeException("Cannot stream the content for => " + document.getCode()); // Throw
																											// exception
																											// if
																											// content
																											// stream is
	...
	/**
	 * Copies the content of the input stream to the response output, handles
	...
	private long copyOutput(InputStream is, HttpServletResponse response, String encoding, String contentType,
		...
		if (encoding == null || encoding.trim().length() == 0) {
			...
			ByteArrayInputStream bis = new ByteArrayInputStream(Base64.encodeBase64(buffer)); // Encode content in
																								// base64
