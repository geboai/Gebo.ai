 ...
 * <p>
 * The {@code GBaseMessageBroker} class acts as the base implementation of a
...
public class GBaseMessageBroker implements IGMessageBroker {
	 ...
	 *
	 * @return {@code true} if every payload type is accepted, otherwise
	 *         {@code false}.
	 */
	@Override
	...
	/**
	 * Adds a system component like a receiver or emitter to the broker. Ensures the
	 * component is not already registered.
	 *
	...
	/**
	 * Removes a system component from the broker. Checks if the component is
	...
	/**
	 * Accepts a message envelope and routes it to the appropriate receiver if
	 * conditions are met. Validates the receiver and emitter capabilities based on
	 * the envelope's payload type.
	 *
	...
	/**
	 * Checks if a receiving component with a specific module and component ID is
	 * present.
	 *
	...
	public void broadcast(GMessageEnvelope envelope) {
		final String payloadType = envelope.getPayloadType();
		if (payloadType != null && payloadType.trim().length() > 0) {
			for (Map<String, IGMessageReceiver> receiverMap : receivers.values()) {
				for (IGMessageReceiver receiver : receiverMap.values()) {
					if (receiver.handlesMessagePayload(envelope.getPayload())
							&& receiver.getAcceptedPayloadTypes().contains(payloadType)) {
						try {
							receiver.accept(envelope);
						} catch (Throwable th) {
							LOGGER.error("Error broadcasting a message to:" + receiver.getCompleteId(), th);
						}
					}
				}
			}
