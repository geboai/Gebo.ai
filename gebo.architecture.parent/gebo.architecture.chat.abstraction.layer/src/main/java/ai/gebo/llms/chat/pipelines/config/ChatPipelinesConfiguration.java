	...
	public ChatPipelinesConfiguration() {
		...
		this.defaultPipelineRoutingDecisionPrompt.setPrompt(
				"You are a chat pipeline routing manager. Your task is to output ONLY a single JSON object (no markdown, no extra text).\r\n"
						...
						+ "   Important: \"explain in detail\" or \"long explanation\" alone is sufficient to choose deep search ONLY if no data regarding the user question is provided in the document list.\r\n"
						+ "3) RAG_LLM_RESPONSE:\r\n"
						+ "   Choose this if the answer is not fully supported by training knowledge OR the currently provided in the document list, and retrieval from a knowledge base is likely sufficient (no deep multi-source analysis required).\r\n"
						+ "4) PURE_LLM_RESPONSE:\r\n"
						...
						+ "- queryRewritings: fill both semantic search query rewrite and full text keywords search query rewrite ONLY when responseRoutingDecision is RAG_LLM_RESPONSE .\r\n"
