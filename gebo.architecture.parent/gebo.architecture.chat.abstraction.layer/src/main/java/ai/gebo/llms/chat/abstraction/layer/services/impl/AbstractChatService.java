 ...
 * Provides an abstract implementation of chat services, facilitating integration with different chat models.
 */
public abstract class AbstractChatService implements IGGenericalChatService {
    protected final static ObjectMapper mapper = new ObjectMapper(); // JSON object mapper for serialization/deserialization
    ...
    protected static class DocumentMessage extends SystemMessage {
        ...
        private static String renderData(Document document) {
            ...
            if (document.getMetadata() != null) {
                ...
                if (id != null) meta.put("fragment-id", id); // Always useful
                if (code != null) meta.put("content-code", code);
                if (title != null) meta.put("title", title);
                if (subtitle != null) meta.put("subtitle", subtitle);
                if (metadata != null) meta.put("tags", metadata);
                if (page != null) meta.put("page_hint", page);

                // Emit a block that starts with ===META=== and ends with ===ENDMETA===.
                if (!meta.isEmpty()) {
                    data.append("===META===\n");
                    meta.forEach((k, v) -> {
                        if (v != null && !v.toString().isBlank()) data.append(k).append(": ").append(v).append('\n');
                    });
    ...
    /**
     * Converts the given query response object to a string, handling JSON processing exceptions.
     *
    ...
    /**
     * Calls the chat client, processes the chat response, and updates the associated GeboChatResponse.
     *
    ...
    /**
     * Calls the chat client for templated responses and updates the associated GeboTemplatedChatResponse.
     *
     * @param configurableChatModel Configurable chat model
     * @param prompt Chat prompt
     * @param context Knowledge base context
    ...
    /**
     * Streams chat response and finalizes the GUserChatContext update when finished.
     *
     * @param configurableChatModel Configurable chat model
     * @param prompt Chat prompt 
     * @param context Knowledge base context
    ...
    protected Flux<GeboChatMessageEnvelope> streamChatClient(IGConfigurableChatModel configurableChatModel,
        ...
        Flux<ChatResponse> res = client.prompt(prompt).user(request.getQuery()).messages(getMessages(messages, docs))
                .toolContext(toolsContext).stream().chatResponse();

        return composeFlux(res, context, request, response, userContext, docrefs, toolsContext);
    }
    ...
    protected Flux<GeboChatMessageEnvelope> composeFlux(Flux<ChatResponse> res, final KBContext context,
            ...
            final List<GResponseDocumentRef> docrefs, Map<String, Object> toolsContext) {
    ...
    /**
     * Converts a collection of ToolCall objects to a list of CalledFunction objects.
     *
