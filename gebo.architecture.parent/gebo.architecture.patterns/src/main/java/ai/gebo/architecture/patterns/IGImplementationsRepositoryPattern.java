 ...
 * 
 * This interface defines a repository pattern for managing implementations of a
 * certain interface. It provides methods to retrieve, search, map, and add
 * implementations.
 *
 * @param <ImplementedInterface> The interface type that implementations will
 *                               adhere to.
 */
public interface IGImplementationsRepositoryPattern<ImplementedInterface> {
	...
	/**
	 * Finds a specific implementation of the interface using a given predicate
	 * filter.
	 * 
	...
	public default ImplementedInterface findByCode(String code) {
		return findImplementation((x) -> {
			return (getCodeValue(x) != null && code != null && getCodeValue(x).equals(code)) || code == getCodeValue(x);
		});
	}

	public default ImplementedInterface findByNullCode() {
