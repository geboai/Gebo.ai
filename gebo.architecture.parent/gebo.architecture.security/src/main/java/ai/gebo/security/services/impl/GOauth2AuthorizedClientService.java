...
/**
 * Implements the OAuth2AuthorizedClientService to manage storing and retrieving OAuth2 tokens using a secret management service.
 */
public class GOauth2AuthorizedClientService implements OAuth2AuthorizedClientService {
    ...
    /**
     * Constructs the service with the required client registration repository and secrets access service.
     * 
     * @param clientRegistrationRepository the repository for client registrations
     * @param secrets the secret access service to store and retrieve token information
     */
    public GOauth2AuthorizedClientService(ClientRegistrationRepository clientRegistrationRepository, IGeboSecretsAccessService secrets) {
        this.clientRegistrationRepository = clientRegistrationRepository;
        this.secrets = secrets;
    }

    /**
     * Builds a unique secret ID for storing OAuth2 tokens based on principal and registration ID.
     * 
    ...
    /**
     * Saves an OAuth2 client by storing its associated tokens in the secret management service.
     * 
     * @param client the authorized OAuth2 client containing the tokens
     * @param principal the authentication object representing the current principal (user)
     */
    @Override
    ...
    public <T extends OAuth2AuthorizedClient> T loadAuthorizedClient(String clientRegistrationId, String principalName) {
        ...
        try {
            ...
            if (content instanceof GeboOauth2TokenSecretContent tokenContent) {
                // Retrieve client registration and create OAuth2AuthorizedClient from secret content
                ClientRegistration registration = clientRegistrationRepository
                        .findByRegistrationId(clientRegistrationId);
                return (T) new OAuth2AuthorizedClient(registration, principalName, tokenContent.getAccessToken(), tokenContent.getRefreshToken());
            } else
    ...
    /**
     * Removes an authorized OAuth2 client by deleting its associated tokens from the secret management service.
     * 
     * @param registrationId the client registration ID
     * @param principalName the principal (user) name
     */
