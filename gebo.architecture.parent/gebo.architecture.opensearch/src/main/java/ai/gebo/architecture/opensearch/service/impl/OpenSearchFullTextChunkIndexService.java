...
public class OpenSearchFullTextChunkIndexService {
        ...
        while (from < total) {
            ...
            for (FullTextChunk c : batch) {
                ...
                BulkOperation op = new BulkOperation.Builder()
                        .index(i -> i.index(indexName).id(c.getId()).document(doc))
                        .build();
            ...
            if (resp.errors()) {
                ...
                for (BulkResponseItem item : resp.items()) {
                    if (item.error() != null) {
                        failures.add("id=" + item.id() + " type=" + item.error().type() + " reason=" + item.error().reason());
                    }
                }
                int maxLog = Math.min(30, failures.size());
                LOGGER.error("OpenSearch bulk had errors (batch {}-{} of {}, failures={}): {}",
                        from, to, total, failures.size(), failures.subList(0, maxLog));

                throw new RuntimeException("OpenSearch bulk indexing errors. failures=" + failures.size());
            } else {
                LOGGER.info("OpenSearch bulk indexed {} chunks ({}-{} of {}) into index={}",
                        batch.size(), from, to, total, indexName);
            }
    ...
    public void deleteByDocuments(List<FullTextDocument> documents) throws OpenSearchException, IOException {
        ...
        for (FullTextDocument d : documents) {
            if (d == null || isBlank(d.getCode())) continue;
        ...
        while (from < total) {
			...
			DeleteByQueryRequest req = new DeleteByQueryRequest.Builder()
                    ...
                    .refresh(rValue) // consider making configurable (costly at scale)
                    .build();

            DeleteByQueryResponse resp = client.deleteByQuery(req);
            long deleted = resp.deleted() != null ? resp.deleted() : 0L;

            LOGGER.info("OpenSearch deleteByDocuments deleted ~{} chunks for document codes {}-{} of {} (index={})",
    ...
    private static void putStringFromMeta(Map<String, Object> dst, String dstField, Map<String, Object> meta, String metaKey) {
        Object v = meta.get(metaKey);
        if (v == null) return;
        String s = v.toString();
        if (!s.isBlank()) dst.put(dstField, s);
    }

    private static void putIntegerFromMeta(Map<String, Object> dst, String dstField, Map<String, Object> meta, String metaKey) {
        Integer i = toInteger(meta.get(metaKey));
        if (i != null) dst.put(dstField, i);
    }

    private static void putLongFromMeta(Map<String, Object> dst, String dstField, Map<String, Object> meta, String metaKey) {
        Long l = toLong(meta.get(metaKey));
        if (l != null) dst.put(dstField, l);
    }

    private static void putFloatFromMeta(Map<String, Object> dst, String dstField, Map<String, Object> meta, String metaKey) {
        Float f = toFloat(meta.get(metaKey));
        if (f != null) dst.put(dstField, f);
    }
    ...
    private static Float toFloat(Object v) {
        ...
        try { return Float.parseFloat(v.toString().trim()); } catch (Exception e) { return null; }
    }
