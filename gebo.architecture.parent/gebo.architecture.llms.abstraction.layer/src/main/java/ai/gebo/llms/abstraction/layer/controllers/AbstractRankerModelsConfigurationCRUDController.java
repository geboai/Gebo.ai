...
public abstract class AbstractRankerModelsConfigurationCRUDController<RankerModelType extends GBaseRankerModelConfig, ModelChoice extends GBaseRankerModelChoice> {
	...
	protected final IGRankerModelRuntimeConfigurationDao modelRuntimeConfigurationDao;

	// Type of the chat model configuration
	protected final Class<RankerModelType> type;
	 ...
	 */
	protected OperationStatus<RankerModelType> insert(RankerModelType config) {
		SecurityEvent event = securityAuditLoggerService.newSecurityEvent();
		OperationStatus<RankerModelType> status = insertInternal(config);
		logConfigEvent(event, SecurityAuditTaxonomy.Action.LLM_CONFIG_INSERT, config != null ? config.getCode() : null,
				status);
		return status;
	}

	private OperationStatus<RankerModelType> insertInternal(RankerModelType config) {
		RankerModelType out = null;
	 ...
	 */
	protected void handleDefaultModel(RankerModelType config) throws GeboPersistenceException {
		if (config.getDefaultModel() != null && config.getDefaultModel()) {
			List<GBaseRankerModelConfig> all = persistentObjectManager
					.findAllExtendingType(GBaseRankerModelConfig.class);
			for (GBaseRankerModelConfig gBaseChatModelConfig : all) {
	...
	protected OperationStatus<RankerModelType> update(RankerModelType config) {
		SecurityEvent event = securityAuditLoggerService.newSecurityEvent();
		OperationStatus<RankerModelType> status = updateInternal(config);
		logConfigEvent(event, SecurityAuditTaxonomy.Action.LLM_CONFIG_UPDATE, config != null ? config.getCode() : null,
				status);
		return status;
	}

	private OperationStatus<RankerModelType> updateInternal(RankerModelType config) {
		try {
	...
	private OperationStatus<Boolean> deleteInternal(RankerModelType type) {
		try {
	 ...
	 * @throws GeboPersistenceException If a persistence error occurs.
	 */
	protected RankerModelType findByCode(String code) throws GeboPersistenceException {
	 ...
	 */
	protected abstract OperationStatus<List<ModelChoice>> getModelChoices(RankerModelType config); 
}