...
/**
 * RagDocumentCacheItem class represents a cache item for a document in a
 * retrieval-augmented generation system. It extends GBaseObject and is
 * annotated for MongoDB document storage. It stores the document's text, token
 * size, byte size, and last modification time. AI generated comments
 */
...
public class AIDocumentCacheItem extends GBaseObject {
	...
	/**
	 * Factory method to create a RagDocumentCacheItem from a stream of Document
	 * objects. Aggregates the text from each Document and calculates token and byte
	 * sizes.
	 *
	...
	private static Map<String, Object> add(Map<String, Object>... m1) {
	...
	public static AIDocumentCacheItem of(GeboDocument geboDocument) {
		...
		if (geboDocument.getId() != null) {
			metas.put(DocumentMetaInfos.CONTENT_CODE, geboDocument.getId());
		}
		if (geboDocument.getName() != null) {
			metas.put(DocumentMetaInfos.GEBO_FILE_NAME, geboDocument.getName());
		}
		if (geboDocument.getContentType() != null) {
			metas.put(DocumentMetaInfos.CONTENT_TYPE, geboDocument.getContentType());
		}
		if (geboDocument.getExtension() != null) {
			metas.put(DocumentMetaInfos.CONTENT_EXTENSION, geboDocument.getExtension());
		}
		Map<String, Object> additional = geboDocument.getAdditionalAttributes();
		Map<String, Object> custom = geboDocument.getCustomMetaData();
		final Map<String, Object> joined = add(metas, additional, custom);
		Stream<Document> stream = (geboDocument.getTexts().stream()
