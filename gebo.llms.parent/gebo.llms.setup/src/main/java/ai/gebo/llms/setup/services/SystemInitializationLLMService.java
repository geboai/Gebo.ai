...
@Component
@Scope("singleton")
public class SystemInitializationLLMService implements ApplicationListener<ContextRefreshedEvent> {
						...
						if (vendor.getProviderId() == null || vendor.getProviderId().trim().length() == 0) {
							...
							Optional<LLMSVendor> foundvendor = vendorsLibraryConfiguration.getVendors().stream()
									.filter(x -> x.getVendorInfo() != null && x.getVendorInfo().getVendorId() != null
											&& x.getVendorInfo().getVendorId().equalsIgnoreCase(vendor.getProviderId()))
									.findFirst();
							if (foundvendor.isEmpty()) {
								...
								String chatModelCode = chatModelConfig != null ? chatModelConfig.getModelCode() : null;
								...
								Optional<LLMSModelsPresets> chatPreset = vendorConfigurationData.getPresets().stream()
										.filter(x -> x.getType() == ModelType.CHAT).findFirst();
								...
								if (embeddingModelCode != null) {
									...
									embeddingData.setSecretId(createdSecret != null ? createdSecret.getCode() : null);
									embeddingData.setModelCode(chatModelCode);
