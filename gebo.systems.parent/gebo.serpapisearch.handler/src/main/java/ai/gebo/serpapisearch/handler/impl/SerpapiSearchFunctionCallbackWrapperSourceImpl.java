...
/** Exposes SerpApi web search as an internet-browsing LLM tool, if configured. */
@Service
public class SerpapiSearchFunctionCallbackWrapperSourceImpl implements IGToolCallbackSource {

	@Autowired
	SerpapiSearchConfigDaoImpl dao;
	@Autowired
	SerpapiSearchApi serpapiApi;
	...
	public List<ToolCallback> getToolCallbacks() {
		...
		List<SerpapiSearchConfig> configurations = dao.getConfigurations();
		if (!configurations.isEmpty()) {
			out.add(serpapiApi.create(configurations.get(0)));
		}
		return out;
	}
