...
/**
 * The Userspace microservice's own instance of the job-status replicator,
 * registered under Userspace's own already-owned
 * {@link GStandardModulesConstraints#USERSPACE_MODULE}, so it is uniquely
...
public class UserspaceJobStatusReplicatorService extends AbstractJobStatusReplicatorService {

	public UserspaceJobStatusReplicatorService(GeboApplicationArchitecture architecture,
			IGReplicationsMap replicationsMap, IMessageEnvelopeFactory envelopeFactory, IGRuntimeBinder runtimeBinder) {
		super(GStandardModulesConstraints.USERSPACE_MODULE, architecture, replicationsMap, envelopeFactory, runtimeBinder);
	}
}
