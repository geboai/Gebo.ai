...
@Data
public class SearxngSearchHandlerConfig implements IGStaticPromptsProvider, IGStaticPromptUseInfoProvider {
	...
	public List<GPromptUseInfo> uses() {
		...
		info.setCode(SEARXNG_SEARCH_QUERY_EXTRACTION_PROMPT);
		info.setDescription(
				"Plans a native SearXNG query (search strings plus optional categories/timeRange/language options) from the user's question and the consolidated knowledge gathered so far.");
		info.setModule("searxngsearch");
