...
/**
 * The Filesystem microservice's own instance of the job-status replicator,
 * registered under Filesystem's own already-owned
 * {@link GStandardModulesConstraints#SHARED_FILESYSTEM_MODULE}, so it is uniquely
...
public class FilesystemJobStatusReplicatorService extends AbstractJobStatusReplicatorService {

	public FilesystemJobStatusReplicatorService(GeboApplicationArchitecture architecture,
			IGReplicationsMap replicationsMap, IMessageEnvelopeFactory envelopeFactory, IGRuntimeBinder runtimeBinder) {
		super(GStandardModulesConstraints.SHARED_FILESYSTEM_MODULE, architecture, replicationsMap, envelopeFactory, runtimeBinder);
	}
}
