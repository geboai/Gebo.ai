...
/**
 * The Integration microservice's own instance of the job-status replicator,
 * registered under Integration's own already-owned
 * {@link GStandardModulesConstraints#INTEGRATION_MODULE}, so it is uniquely
...
public class IntegrationJobStatusReplicatorService extends AbstractJobStatusReplicatorService {

	public IntegrationJobStatusReplicatorService(GeboApplicationArchitecture architecture,
			IGReplicationsMap replicationsMap, IMessageEnvelopeFactory envelopeFactory, IGRuntimeBinder runtimeBinder) {
		super(GStandardModulesConstraints.INTEGRATION_MODULE, architecture, replicationsMap, envelopeFactory, runtimeBinder);
	}
}
