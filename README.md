...
- **Wazuh-compatible JSON format** — each log line is a single, complete JSON object with no timestamp/level prefix, ready for Wazuh's (or any SIEM's) `json` log_format decoder: point your agent at `security-log.json` and start ingesting.
