 ...
 * This module manages OpenAI chat model configurations in the Gebo.ai application.
...
import { FunctionsLookupControllerService, GBaseChatModelChoice, GLookupEntry, GOpenAIChatModelConfig, OpenAiChatModelsConfigurationControllerService } from '@Gebo.ai/brain';
...
export class GeboAIOpenAIChatModelAdminComponent extends BaseEntityEditingComponentAutoDeleteCheck<GOpenAIChatModelConfig> {
     ...
     */
    protected override entityName: string = "GOpenAIChatModelConfig";
    ...
    constructor(injector: Injector, geboFormGroupsService: GeboFormGroupsService,
        private openaiChatModelConfigService: OpenAiChatModelsConfigurationControllerService,
    ...
    private loadModels(newValue: { baseUrl?: string, apiSecretCode?: string }) {
        this.loadingRelatedBackend = true;
        this.openaiChatModelConfigService.getOpenAIChatModels(newValue).subscribe({
            next: (r) => {
     ...
     */
    protected override onLoadedPersistentData(actualValue: GOpenAIChatModelConfig): void {
        this.formGroup.controls["modelTypeCode"].setValue("chatgpt-OpenAI");
     ...
     */
    protected override onNewData(actualValue: GOpenAIChatModelConfig): void {
        this.formGroup.controls["modelTypeCode"].setValue("chatgpt-OpenAI");
    }
     ...
     */
    override findByCode(code: string): Observable<GOpenAIChatModelConfig | null> {
        return this.openaiChatModelConfigService.findOpenAIChatModelConfigByCode(code);
    }
     ...
     */
    override save(value: GOpenAIChatModelConfig): Observable<GOpenAIChatModelConfig> {
        return this.openaiChatModelConfigService.updateOpenAIChatModelConfig(value).pipe(map(result => {
     ...
     */
    override insert(value: GOpenAIChatModelConfig): Observable<GOpenAIChatModelConfig> {
        return this.openaiChatModelConfigService.insertOpenAIChatModelConfig(value).pipe(map(result => {
            this.updateLastOperationStatus(result);
            if (!result?.result) return value;
            return result.result;
        }));
     ...
     */
    override delete(value: GOpenAIChatModelConfig): Observable<boolean> {
        return this.openaiChatModelConfigService.deleteOpenAIChatModelConfig(value).pipe(map(result => {
