 ...
 * This component is responsible for managing OpenAI embedding model configurations.
 * It extends BaseEntityEditingComponent to handle CRUD operations for GOpenAIEmbeddingModelConfig entities.
...
import { GBaseChatModelChoice, GOpenAIEmbeddingModelConfig, OpenAiEmbeddingModelsConfigurationControllerService, SecretInfo, SecretsControllerService } from "@Gebo.ai/gebo-ai-rest-api";
...
@Component({
    selector: "gebo-ai-open-ai-embed-model-admin-component",
    templateUrl: "gebo-ai-openai-embedmodel-admin.component.html",
    standalone: false, providers: [{
        provide: GEBO_AI_FIELD_HOST, useExisting: forwardRef(() => GeboAIOpenAIEmbedModelAdminComponent),
        multi: true
    }]

})
export class GeboAIOpenAIEmbedModelAdminComponent extends BaseEntityEditingComponent<GOpenAIEmbeddingModelConfig> {
    /**
     * The entity name identifier for the GOpenAIEmbeddingModelConfig
     */
    protected override entityName: string = "GOpenAIEmbeddingModelConfig";
    ...
    constructor(injector:Injector,geboFormGroupsService: GeboFormGroupsService,
        private openaiEmbedModelConfigService: OpenAiEmbeddingModelsConfigurationControllerService,
    ...
    private loadModels(newValue: { baseUrl?: string, apiSecretCode?: string }) {
        ...
        this.openaiEmbedModelConfigService.getOpenAIEmbeddingModels(newValue).subscribe({
            next: (r) => {
     ...
     */
    protected override onLoadedPersistentData(actualValue: GOpenAIEmbeddingModelConfig): void {
        this.formGroup.controls["modelTypeCode"].setValue("embedding-OpenAI");
     ...
     */
    protected override onNewData(actualValue: GOpenAIEmbeddingModelConfig): void {
        this.formGroup.controls["modelTypeCode"].setValue("embedding-OpenAI");
    }
     ...
     */
    override findByCode(code: string): Observable<GOpenAIEmbeddingModelConfig | null> {
        return this.openaiEmbedModelConfigService.findOpenAIEmbeddingModelConfigByCode(code)
    }
     ...
     */
    override save(value: GOpenAIEmbeddingModelConfig): Observable<GOpenAIEmbeddingModelConfig> {
        
        return this.openaiEmbedModelConfigService.updateOpenAIEmbeddingModelConfig(value).pipe(map(result => {
     ...
     */
    override insert(value: GOpenAIEmbeddingModelConfig): Observable<GOpenAIEmbeddingModelConfig> {
        
        return this.openaiEmbedModelConfigService.insertOpenAIEmbeddingModelConfig(value).pipe(map(result => {
            this.updateLastOperationStatus(result);
            if (!result?.result) return value;
            return result.result;
        }));
     ...
     */
    override delete(value: GOpenAIEmbeddingModelConfig): Observable<boolean> {
        return this.openaiEmbedModelConfigService.deleteOpenAIEmbeddingModelConfig(value).pipe(map(result => {
     ...
     */
    override canBeDeleted(value: GOpenAIEmbeddingModelConfig): Observable<{ canBeDeleted: boolean; message: string; }> {
