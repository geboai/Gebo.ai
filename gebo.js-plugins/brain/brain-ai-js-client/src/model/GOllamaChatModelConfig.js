/*
 * OpenAPI definition
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v0
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 3.0.59
 *
 * Do not edit the class manually.
 *
 */
import ApiClient from '../ApiClient';
import GOllamaChatModelChoice from './GOllamaChatModelChoice';

/**
 * The GOllamaChatModelConfig model module.
 * @module model/GOllamaChatModelConfig
 * @version v0
 */
export default class GOllamaChatModelConfig {
  /**
   * Constructs a new <code>GOllamaChatModelConfig</code>.
   * @alias module:model/GOllamaChatModelConfig
   * @class
   */
  constructor() {
  }

  /**
   * Constructs a <code>GOllamaChatModelConfig</code> from a plain JavaScript object, optionally creating a new instance.
   * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
   * @param {Object} data The plain JavaScript object bearing properties of interest.
   * @param {module:model/GOllamaChatModelConfig} obj Optional instance to populate.
   * @return {module:model/GOllamaChatModelConfig} The populated <code>GOllamaChatModelConfig</code> instance.
   */
  static constructFromObject(data, obj) {
    if (data) {
      obj = obj || new GOllamaChatModelConfig();
      if (data.hasOwnProperty('code'))
        obj.code = ApiClient.convertToType(data['code'], 'String');
      if (data.hasOwnProperty('description'))
        obj.description = ApiClient.convertToType(data['description'], 'String');
      if (data.hasOwnProperty('userModified'))
        obj.userModified = ApiClient.convertToType(data['userModified'], 'String');
      if (data.hasOwnProperty('userCreated'))
        obj.userCreated = ApiClient.convertToType(data['userCreated'], 'String');
      if (data.hasOwnProperty('dateModified'))
        obj.dateModified = ApiClient.convertToType(data['dateModified'], 'Date');
      if (data.hasOwnProperty('dateCreated'))
        obj.dateCreated = ApiClient.convertToType(data['dateCreated'], 'Date');
      if (data.hasOwnProperty('modelTypeCode'))
        obj.modelTypeCode = ApiClient.convertToType(data['modelTypeCode'], 'String');
      if (data.hasOwnProperty('defaultModel'))
        obj.defaultModel = ApiClient.convertToType(data['defaultModel'], 'Boolean');
      if (data.hasOwnProperty('apiSecretCode'))
        obj.apiSecretCode = ApiClient.convertToType(data['apiSecretCode'], 'String');
      if (data.hasOwnProperty('choosedModel'))
        obj.choosedModel = GOllamaChatModelChoice.constructFromObject(data['choosedModel']);
      if (data.hasOwnProperty('baseUrl'))
        obj.baseUrl = ApiClient.convertToType(data['baseUrl'], 'String');
      if (data.hasOwnProperty('contextLength'))
        obj.contextLength = ApiClient.convertToType(data['contextLength'], 'Number');
      if (data.hasOwnProperty('topP'))
        obj.topP = ApiClient.convertToType(data['topP'], 'Number');
      if (data.hasOwnProperty('accessibleGroups'))
        obj.accessibleGroups = ApiClient.convertToType(data['accessibleGroups'], ['String']);
      if (data.hasOwnProperty('accessibleUsers'))
        obj.accessibleUsers = ApiClient.convertToType(data['accessibleUsers'], ['String']);
      if (data.hasOwnProperty('accessibleToAll'))
        obj.accessibleToAll = ApiClient.convertToType(data['accessibleToAll'], 'Boolean');
      if (data.hasOwnProperty('enabledFunctions'))
        obj.enabledFunctions = ApiClient.convertToType(data['enabledFunctions'], ['String']);
      if (data.hasOwnProperty('temperature'))
        obj.temperature = ApiClient.convertToType(data['temperature'], 'Number');
      if (data.hasOwnProperty('forUses'))
        obj.forUses = ApiClient.convertToType(data['forUses'], ['String']);
      if (data.hasOwnProperty('features'))
        obj.features = ApiClient.convertToType(data['features'], ['String']);
      if (data.hasOwnProperty('thinking'))
        obj.thinking = ApiClient.convertToType(data['thinking'], 'String');
      if (data.hasOwnProperty('maxGeneratedTokens'))
        obj.maxGeneratedTokens = ApiClient.convertToType(data['maxGeneratedTokens'], 'Number');
    }
    return obj;
  }
}

/**
 * @member {String} code
 */
GOllamaChatModelConfig.prototype.code = undefined;

/**
 * @member {String} description
 */
GOllamaChatModelConfig.prototype.description = undefined;

/**
 * @member {String} userModified
 */
GOllamaChatModelConfig.prototype.userModified = undefined;

/**
 * @member {String} userCreated
 */
GOllamaChatModelConfig.prototype.userCreated = undefined;

/**
 * @member {Date} dateModified
 */
GOllamaChatModelConfig.prototype.dateModified = undefined;

/**
 * @member {Date} dateCreated
 */
GOllamaChatModelConfig.prototype.dateCreated = undefined;

/**
 * @member {String} modelTypeCode
 */
GOllamaChatModelConfig.prototype.modelTypeCode = undefined;

/**
 * @member {Boolean} defaultModel
 */
GOllamaChatModelConfig.prototype.defaultModel = undefined;

/**
 * @member {String} apiSecretCode
 */
GOllamaChatModelConfig.prototype.apiSecretCode = undefined;

/**
 * @member {module:model/GOllamaChatModelChoice} choosedModel
 */
GOllamaChatModelConfig.prototype.choosedModel = undefined;

/**
 * @member {String} baseUrl
 */
GOllamaChatModelConfig.prototype.baseUrl = undefined;

/**
 * @member {Number} contextLength
 */
GOllamaChatModelConfig.prototype.contextLength = undefined;

/**
 * @member {Number} topP
 */
GOllamaChatModelConfig.prototype.topP = undefined;

/**
 * @member {Array.<String>} accessibleGroups
 */
GOllamaChatModelConfig.prototype.accessibleGroups = undefined;

/**
 * @member {Array.<String>} accessibleUsers
 */
GOllamaChatModelConfig.prototype.accessibleUsers = undefined;

/**
 * @member {Boolean} accessibleToAll
 */
GOllamaChatModelConfig.prototype.accessibleToAll = undefined;

/**
 * @member {Array.<String>} enabledFunctions
 */
GOllamaChatModelConfig.prototype.enabledFunctions = undefined;

/**
 * @member {Number} temperature
 */
GOllamaChatModelConfig.prototype.temperature = undefined;

/**
 * Allowed values for the <code>forUses</code> property.
 * @enum {String}
 * @readonly
 */
GOllamaChatModelConfig.ForUsesEnum = {
  /**
   * value: "CHAT"
   * @const
   */
  CHAT: "CHAT",

  /**
   * value: "INTERNAL_SERVICES"
   * @const
   */
  INTERNAL_SERVICES: "INTERNAL_SERVICES"
};
/**
 * @member {Array.<module:model/GOllamaChatModelConfig.ForUsesEnum>} forUses
 */
GOllamaChatModelConfig.prototype.forUses = undefined;

/**
 * Allowed values for the <code>features</code> property.
 * @enum {String}
 * @readonly
 */
GOllamaChatModelConfig.FeaturesEnum = {
  /**
   * value: "CHAT"
   * @const
   */
  CHAT: "CHAT",

  /**
   * value: "REASONING"
   * @const
   */
  REASONING: "REASONING",

  /**
   * value: "STRUCTURED_OUTPUT"
   * @const
   */
  STRUCTURED_OUTPUT: "STRUCTURED_OUTPUT",

  /**
   * value: "MULTIMEDIA"
   * @const
   */
  MULTIMEDIA: "MULTIMEDIA",

  /**
   * value: "FUNCTION_CALLING"
   * @const
   */
  FUNCTION_CALLING: "FUNCTION_CALLING"
};
/**
 * @member {Array.<module:model/GOllamaChatModelConfig.FeaturesEnum>} features
 */
GOllamaChatModelConfig.prototype.features = undefined;

/**
 * Allowed values for the <code>thinking</code> property.
 * @enum {String}
 * @readonly
 */
GOllamaChatModelConfig.ThinkingEnum = {
  /**
   * value: "NO_THINKING"
   * @const
   */
  NO_THINKING: "NO_THINKING",

  /**
   * value: "LOW_THINKING"
   * @const
   */
  LOW_THINKING: "LOW_THINKING",

  /**
   * value: "MEDIUM_THINKING"
   * @const
   */
  MEDIUM_THINKING: "MEDIUM_THINKING",

  /**
   * value: "HIGH_THINKING"
   * @const
   */
  HIGH_THINKING: "HIGH_THINKING",

  /**
   * value: "AUTO"
   * @const
   */
  AUTO: "AUTO"
};
/**
 * @member {module:model/GOllamaChatModelConfig.ThinkingEnum} thinking
 */
GOllamaChatModelConfig.prototype.thinking = undefined;

/**
 * @member {Number} maxGeneratedTokens
 */
GOllamaChatModelConfig.prototype.maxGeneratedTokens = undefined;

